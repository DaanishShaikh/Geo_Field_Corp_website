<template>
  <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
    <div class="bg-[#061e12] border border-emerald-800/80 rounded-2xl w-full max-w-lg p-6 sm:p-8 shadow-2xl space-y-6 relative max-h-[90vh] overflow-y-auto text-white">
      <!-- Close Button -->
      <button 
        @click="$emit('close')" 
        class="absolute right-5 top-5 text-emerald-400/80 hover:text-white p-1.5 rounded-xl hover:bg-emerald-900/50 transition cursor-pointer"
        aria-label="Close"
      >
      ...
      <div class="text-center space-y-1.5">
        <div class="w-10 h-10 rounded-2xl bg-gradient-to-tr from-emerald-600 via-green-500 to-teal-400 flex items-center justify-center font-black text-white text-lg mx-auto shadow-lg shadow-emerald-950/50">
          G
        </div>
        <h2 class="text-xl font-black text-white">GeoField RUCO Trace</h2>
        <p class="text-xs text-emerald-300/80">National Used Cooking Oil Compliance & Traceability System</p>
      </div>

      <!-- Tab Switcher -->
      <div class="grid grid-cols-2 bg-[#030e08] p-1 rounded-2xl border border-emerald-900">
        <button 
          @click="mode = 'login'" 
          :class="mode === 'login' ? 'bg-emerald-800/80 text-white font-bold shadow' : 'text-emerald-400/80 hover:text-emerald-200 font-medium'"
        ...
        <button 
          @click="mode = 'register'" 
          :class="mode === 'register' ? 'bg-emerald-800/80 text-white font-bold shadow' : 'text-emerald-400/80 hover:text-emerald-200 font-medium'"
          class="py-2 text-xs rounded-xl transition cursor-pointer"
        >
          New Onboarding
        </button>
      ...
      <form v-if="mode === 'login'" @submit.prevent="handleLogin" class="space-y-4">
        <div>
          <label class="block text-xs font-semibold text-emerald-200/80 mb-1.5">Registered Email Address</label>
          <input 
            v-model="loginForm.email" 
            type="email" 
            required 
            placeholder="e.g. contact@restaurant.com" 
            class="w-full bg-[#030e08] border border-emerald-900/80 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500 transition"
          />
        </div>

        <div>
          <label class="block text-xs font-semibold text-emerald-200/80 mb-1.5">Password</label>
          <input 
        ...
        <div class="p-2.5 bg-[#030e08] border border-emerald-900/80 rounded-xl flex items-center justify-between gap-2">
          <div class="text-[11px] text-emerald-300/80">
            <span class="text-white font-semibold">Super Admin:</span> <code class="text-emerald-400 font-mono">admin@geofield.com</code>
          </div>
          <button 
            type="button" 
            @click="fillAdminCredentials"
            class="px-2.5 py-1 bg-emerald-950/70 hover:bg-emerald-900 text-emerald-300 hover:text-white rounded-lg text-[10px] font-bold transition border border-emerald-800/80 cursor-pointer"
          >
            Fill Admin
          </button>
        </div>
        ...
        <button 
          type="submit" 
          :disabled="loading"
          class="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-emerald-950/50 hover:shadow-emerald-900/40 transition disabled:opacity-50 cursor-pointer"
        >
          {{ loading ? 'Authenticating...' : 'Sign In to Portal' }}
        </button>

        <p class="text-[11px] text-center text-emerald-300/70 pt-2">
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-4">
        <!-- Role Switcher -->
        <div>
          <label class="block text-xs font-semibold text-emerald-200/80 mb-1">Entity / Role Type</label>
          <div class="grid grid-cols-2 gap-2">
            <label 
              :class="regForm.role === 'seller' ? 'bg-emerald-950/60 border-emerald-500 text-emerald-300 font-bold' : 'bg-[#030e08] border-emerald-900/80 text-emerald-400/60'"
            ...
            <label 
              :class="regForm.role === 'agent' ? 'bg-emerald-950/60 border-emerald-500 text-emerald-300 font-bold' : 'bg-[#030e08] border-emerald-900/80 text-emerald-400/60'"
              class="p-2.5 rounded-xl border flex items-center justify-center text-xs cursor-pointer transition"
            >
        ...
        <div v-if="regForm.role === 'seller'" class="space-y-2">
          ...
          <div class="flex items-center justify-between text-xs font-semibold text-emerald-200/80">
            <span class="flex items-center gap-1.5 text-emerald-400 font-mono">
              <span class="text-green-300">Step {{ currentStep }} of 3:</span>
              <span class="text-white">{{ stepTitles[currentStep - 1] }}</span>
            </span>
            <span class="text-[11px] font-mono text-emerald-400/70">{{ Math.round((currentStep / 3) * 100) }}% Completed</span>
          </div>

          <!-- Progress Bar Track -->
          <div class="w-full h-2 bg-[#030e08] rounded-full overflow-hidden border border-emerald-900/80">
            <div 
              class="h-full bg-gradient-to-r from-emerald-500 via-green-400 to-teal-400 transition-all duration-300 ease-out"
        ...
        <div v-if="regForm.role === 'seller' && currentStep === 1" class="space-y-3 animate-fade-in">
          <div class="p-3 bg-emerald-950/60 border border-emerald-800/60 rounded-xl text-[11px] text-emerald-300 flex items-center gap-2">
            <span>🛡️</span>
          ...
          <div class="space-y-2.5">
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">
              ...
              <input 
                ...
                class="w-full bg-[#030e08] border border-emerald-900/80 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-500"
              />
            </div>
          ...
          <!-- Step 1 Actions -->
          <div class="pt-3 border-t border-emerald-900/80 flex justify-end">
            <button 
              type="button" 
              @click="goToStep(2)"
        ...
        <div v-if="regForm.role === 'seller' && currentStep === 2" class="space-y-3 animate-fade-in">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input 
                  ...
                  class="w-full bg-[#030e08] border border-emerald-900/80 rounded-xl px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-emerald-500"
                />
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  class="w-full bg-[#030e08] border border-emerald-900/80 rounded-xl px-3 py-2 text-xs text-white font-mono uppercase focus:outline-none focus:border-emerald-500"
                />
              </div>
            </div>
          ...
          <!-- Step 2 Actions -->
          <div class="pt-3 border-t border-emerald-900/80 flex items-center justify-between gap-2">
            <button 
              type="button" 
              @click="currentStep = 1"
              class="px-4 py-2 bg-emerald-950/70 hover:bg-emerald-900 text-emerald-300 text-xs font-semibold rounded-xl border border-emerald-800/80 transition cursor-pointer"
            >
              &larr; Back
            </button>
            <button 
        ...
        <div v-if="regForm.role === 'seller' && currentStep === 3" class="space-y-3 animate-fade-in">
          ...
          <div class="space-y-2.5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">City / Region <span class="text-rose-400">*</span></label>
                <input 
              ...
              <div>
                <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Pincode <span class="text-rose-400">*</span></label>
                <input 
            ...
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Preferred Pickup Window</label>
              <select 
          ...
          <div class="p-3 bg-[#030e08] border border-emerald-900/80 rounded-2xl space-y-2.5">
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                <label class="block text-[10px] font-semibold text-emerald-400/80 mb-0.5">Latitude</label>
                <input 
                  v-model.number="regForm.latitude" 
                  type="number" 
                  step="any" 
                  placeholder="e.g. 19.0760" 
                  class="w-full bg-[#05180f] border border-emerald-900/80 rounded-xl px-3 py-1.5 text-xs text-white font-mono focus:outline-none focus:border-emerald-500"
                />
              </div>
              <div>
                <label class="block text-[10px] font-semibold text-emerald-400/80 mb-0.5">Longitude</label>
                <input 
          ...
          <div class="p-3 rounded-xl bg-[#030e08] border border-emerald-900/80 text-[11px] text-emerald-300/80 leading-relaxed">
          ...
          <div class="pt-3 border-t border-emerald-900/80 flex items-center justify-between gap-2">
            ...
            <button 
              type="submit" 
              :disabled="loading"
              class="px-6 py-2.5 bg-gradient-to-r from-emerald-600 to-green-500 hover:from-emerald-500 hover:to-green-400 text-white font-bold text-xs rounded-xl shadow-lg transition disabled:opacity-50 cursor-pointer"
            >
        ...
        <div v-if="regForm.role === 'agent'" class="space-y-3 animate-fade-in">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Full Legal Name <span class="text-rose-400">*</span></label>
              <input 
                v-model="regForm.name" 
                type="text" 
                required 
            ...
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Official Email <span class="text-rose-400">*</span></label>
              <input 
                v-model="regForm.email" 
                type="email" 
                required 
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Phone Number <span class="text-rose-400">*</span></label>
              <input 
                v-model="regForm.phone" 
                type="text" 
                required 
            ...
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Portal Password <span class="text-rose-400">*</span></label>
              <input 
                v-model="regForm.password" 
                type="password" 
                required 
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Vehicle Registration No. <span class="text-rose-400">*</span></label>
              <input 
            ...
            <div>
              <label class="block text-[11px] font-semibold text-emerald-200/80 mb-1">Operating City / Base <span class="text-rose-400">*</span></label>
              <input 
                v-model="regForm.city" 
                type="text" 
                required 
          ...
          <button 
            type="submit" 
            :disabled="loading"
            class="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-lg transition disabled:opacity-50 cursor-pointer"
          >

<template>
  <div class="bg-slate-800/80 rounded-2xl border border-slate-700/80 p-5 shadow-xl space-y-4">
    ...
    <div class="relative w-full h-[380px] bg-slate-950 rounded-xl overflow-hidden border border-slate-700">
      <div id="mapbox-map-container" class="w-full h-full"></div>

      <!-- Map Floating Overlay Legend -->
      <div class="absolute bottom-3 left-3 bg-slate-900/90 backdrop-blur border border-slate-700/80 rounded-xl p-3 text-xs space-y-1.5 shadow-lg max-w-xs">
...
function initMap() {
  // If Leaflet is loaded in window
  if (window.L) {
    const L = window.L;
    mapInstance = L.map('mapbox-map-container').setView([12.9716, 77.5946], 12);
    
    // High contrast dark/voyager map tiles
...
function renderMarkers() {
      ...
      L.marker([lat, lng], { icon: customIcon })
        ...
        .bindPopup(`
          <div style="font-family:sans-serif;font-size:12px;color:#0f172a;line-height:1.4;">
            <strong>Stop #${stop.stop_order}: ${stop.seller_name}</strong><br>
            <span style="color:#64748b;font-size:10px;">FSSAI: ${stop.seller_fssai || 'N/A'}</span><br>
  ...
  if (props.agents && props.agents.length) {
    props.agents.forEach((agent) => {
      const lat = agent.lat || 12.9716;
      const lng = agent.lng || 77.5946;
      ...
      L.marker([lat, lng], { icon: agentIcon })
        ...
        .bindPopup(`
          <div style="font-family:sans-serif;font-size:12px;color:#0f172a;">
            ...
            <span>Vehicle: ${agent.vehicle_no}</span><br>
            <span style="color:#0284c7;font-weight:bold;">Zone: ${agent.zone}</span>
          </div>
        `);
    });
  }

<template>
  <div class="bg-slate-800/80 rounded-2xl border border-slate-700/80 p-5 shadow-xl space-y-4">
    ...
    <div class="relative w-full h-[380px] bg-slate-950 rounded-xl overflow-hidden border border-slate-700">
      ...
      <div class="absolute bottom-3 left-3 bg-slate-900/90 backdrop-blur border border-slate-700/80 rounded-xl p-3 text-xs space-y-1.5 shadow-lg max-w-xs z-10">
...
function ensureLeafletAndInit() {
  if (window.L) {
    ...
    // Dynamically load Leaflet if not yet present in window
...
function initMap() {
  ...
  // Initialize Map on specific DOM container ref
  ...
  // Force map to calculate container size after render
  setTimeout(() => {
    if (mapInstance) {
...
function renderMarkers() {
  ...
  if (props.stops && props.stops.length) {
    props.stops.forEach((stop, i) => {
      ...
      const customIcon = L.divIcon({
        ...
        html: `<div style="background-color:${markerColor};width:26px;height:26px;border-radius:50%;border:2px solid white;display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;font-size:11px;box-shadow:0 2px 6px rgba(0,0,0,0.5);">${stop.stop_order || i + 1}</div>`,
        iconSize: [26, 26],
        iconAnchor: [13, 13]
      });
      ...
      L.marker([lat, lng], { icon: customIcon })
        ...
        .bindPopup(`
          <div style="font-family:sans-serif;font-size:12px;color:#0f172a;line-height:1.4;">
            <strong>Stop #${stop.stop_order || i + 1}: ${stop.seller_name || 'Collection Point'}</strong><br>
  ...
  if (props.agents && props.agents.length) {
    props.agents.forEach((agent) => {
      ...
      const agentIcon = L.divIcon({
        ...
        html: `<div style="background-color:#0284c7;width:30px;height:30px;border-radius:8px;border:2px solid white;display:flex;align-items:center;justify-content:center;color:white;font-size:15px;box-shadow:0 3px 8px rgba(0,0,0,0.5);">🚚</div>`,
        iconSize: [30, 30],
        iconAnchor: [15, 15]
      });
      ...
      L.marker([lat, lng], { icon: agentIcon })
        ...
        .bindPopup(`
          <div style="font-family:sans-serif;font-size:12px;color:#0f172a;">
            <strong>Agent: ${agent.name || 'Field Officer'}</strong><br>
            <span>Vehicle: ${agent.vehicle_no || 'KA-02-EV-4412'}</span><br>
            <span style="color:#0284c7;font-weight:bold;">Zone: ${agent.zone || 'Central Route'}</span>
          </div>
        `);
    });
  }
  ...
  latlngs.push([refineryLat, refineryLng]);
  
  const refineryIcon = L.divIcon({
    className: 'refinery-div-icon',
    html: `<div style="background-color:#16a34a;width:30px;height:30px;border-radius:8px;border:2px solid white;display:flex;align-items:center;justify-content:center;color:white;font-size:15px;box-shadow:0 3px 8px rgba(0,0,0,0.5);">🏭</div>`,
    iconSize: [30, 30],
    iconAnchor: [15, 15]
  });
  ...
  if (latlngs.length > 1) {
    L.polyline(latlngs, { color: '#0f766e', weight: 3, opacity: 0.8, dashArray: '6, 8' }).addTo(mapInstance);
